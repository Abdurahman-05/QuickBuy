...
export default function Navbar() {
  ...
  return (
    <header className="w-full bg-white border-b border-gray-100 flex flex-col items-center sticky top-0 z-50">
      <div className="flex h-[84px] w-full max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-12">
        ...
        <div className="flex items-center gap-2 sm:gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-3 sm:gap-4 md:gap-6 text-black">
            ...
            {isAuthenticated ? (
                ...
                <div className="flex items-center gap-2 sm:gap-3 border-l pl-2 sm:pl-4 border-gray-100">
                  <Link to="/dashboard" className="flex items-center gap-2 group">
                    <img 
                      ...
                      alt="Profile" 
                      className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover border border-transparent shadow-sm group-hover:border-red-500 transition-all shrink-0"
                    />
                    <span className="text-[13px] font-bold hidden xl:block">{user?.firstName}</span>
                  </Link>
                  <button 
                    onClick={handleLogout}
                    className="p-1 sm:p-2 text-gray-400 hover:text-red-600 transition-colors group/logout"
                    title="Logout"
                  >
                    <svg className="w-[16px] h-[16px] sm:w-[18px] sm:h-[18px] group-hover/logout:-translate-x-0.5 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>
                  </button>
                </div>
              </>
            ) : (
              <Link 
                to="/login" 
                className="bg-black text-white px-5 sm:px-6 py-2 sm:py-2.5 rounded-full text-[12px] sm:text-[13px] font-bold hover:bg-red-600 transition-all shadow-sm"
              >
                Login
              </Link>
            )}
...
function SearchIcon() {
  return (
    <svg className="w-[16px] h-[16px] sm:w-[18px] sm:h-[18px] transition-all" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function ProfileIcon() {
  return (
    <svg className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] transition-all" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function HeartIcon() {
  return (
    <svg className="w-[18px] h-[17px] sm:w-[20px] sm:h-[19px] transition-transform hover:scale-110 active:scale-95" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function CartIcon() {
  return (
    <svg className="w-[18px] h-[17px] sm:w-[24px] sm:h-[22px] transition-transform hover:-translate-y-[2px] active:scale-95" viewBox="0 0 34 30" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
