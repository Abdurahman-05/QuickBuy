...
type NavItem = "Home" | "Shop" | "About";
type ActionItem = "Internet" | "Search" | "Cart" | "Heart" | "Profile";
...
const NAV_ITEMS: NavItem[] = ["Home", "Shop", "About"];
...
export default function Navbar() {
  ...
  const [selectedAction, setSelectedAction] = useState<ActionItem>("Profile");
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const navTextClass =
    "text-[24px] font-medium leading-none tracking-normal text-[#1B1B1B]";

  return (
    <header className="relative bg-[#E9E9E9]">
      <div className="mx-auto flex w-full max-w-[1920px] items-center justify-between gap-4 px-4 sm:px-6 md:px-8 lg:px-[35px] py-[15px]">
        <Logo />

        <nav className="hidden items-center gap-24 lg:flex lg:gap-36">
          {NAV_ITEMS.map((item) => (
            <button
              key={item}
              type="button"
              onClick={() => setSelectedNav(item)}
              className={`relative inline-flex items-center gap-2 ${navTextClass}`}
              style={{ fontFamily: "Inter, sans-serif" }}
            >
              {item === "Home" ? <HomeNavIcon /> : null}
              <span className="relative inline-block">
                {item}
                {selectedNav === item ? (
                  <span className="absolute left-1/2 top-[calc(100%+5px)] h-[7px] w-[27px] -translate-x-1/2 rounded-[20px] bg-[#FF0000]" />
                ) : null}
              </span>
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 md:gap-6 lg:gap-10">
          ...
          {ACTIONS.map((action) => (
            <IconButton
              ...
              className={`hidden md:inline-flex ${action.desktopClassName ?? ""}`}
            >
              {action.icon}
            </IconButton>
          ))}
      ...
      {isMobileMenuOpen ? (
        <div
          ...
          className="border-t border-black/5 bg-[#E9E9E9] px-4 pb-5 pt-4 shadow-[0_16px_30px_rgba(0,0,0,0.08)] md:hidden"
        >
          ...
          <div className="mt-4 grid grid-cols-5 gap-2">
            {ACTIONS.map((action) => (
              <IconButton
                ...
                className="h-12 w-full rounded-full bg-white/70 shadow-sm"
              >
                {action.icon}
              </IconButton>
            ))}
...
function Logo() {
  return (
    <Link to="/" className="inline-flex shrink-0 items-center gap-2 sm:gap-2.5">
      <svg
        width="22"
        height="26"
        viewBox="0 0 22 26"
        ...
        aria-hidden="true"
        className="h-[22px] w-[18px] sm:h-[26px] sm:w-[22px]"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
      ...
      <span
        className="text-[22px] font-bold leading-none tracking-normal text-[#1B1B1B] sm:text-[30px]"
        style={{ fontFamily: "Inter, sans-serif" }}
      >
